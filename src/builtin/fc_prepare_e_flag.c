...
/*   Updated: 2019/09/05 15:06:12 by vde-sain    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static	char		**get_ide_paths(char **paths, char **env)
{
	int				i;
...
void				prepare_e_flag(t_fc *fc, t_hist *hist, t_var **var)
{
	...
	else if (check_if_ename_is_text_editor(fc, paths) == 1)
		send_e_flag_to_exec(fc, hist, env);
	ft_free_tab(paths);
	ft_free_tab(env);
	if (fc->error == 1)
