...
static int	unsetenv_rules(t_process *p)
{
	...
	{
		ft_printf_err_fd("21sh: unsetenv: usage: unsetenv [NAME]\n");
		return (0);
	}
	return (1);
}
	...
	if (*var && p->cmd[i])
	{
		while (p->cmd[i])
		{
			...
			if (remove_list_var(var, ENVIRONEMENT, p->cmd[i]) == 1)
				...
				ft_printf_err_fd("21sh: unsetenv: var %s not found\n",
						p->cmd[i]);
			i++;
		}
		return (0);
		...
		ft_printf_err_fd("21sh: unsetenv: environment not set\n");
	else
		ft_printf_err_fd("21sh: unsetenv: bad parameters, use -u for usage\n");
	return (-1);
}
