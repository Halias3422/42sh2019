...
void	fill_str(char *buf, t_pos *pos)
{
	...
	swap = NULL;
	let_nb = (pos->tot_li - pos->start_li) * pos->max_co + pos->act_co;

	if (let_nb == (int)ft_strlen(pos->ans))
		pos->ans = ft_strjoinf(pos->ans, buf, 0);
	else
	{
		swap = ft_strnew(let_nb + 1);
		swap = ft_strncpy(swap, pos->ans, let_nb);
		swap = ft_strjoinf(swap, buf, 0);
		swap = ft_strjoinf(swap, pos->ans + let_nb, 0);
		//		free(pos->ans);
		pos->ans = swap;
	}
}
